...
def plotFilling(col,time,excess,path = None):
    fig = plt.figure()
    ax = mplot3d.Axes3D(fig)
    liver, mesh = getLiverSTL()
    ax.add_collection3d(liver)
...
def plotRadius(col, path = None):
    ...
    for branch in col.branchList:
        ...
        for j in range(len(coords) - 1):
            ax.plot3D([coords[j][0],coords[j+1][0]],[coords[j][1],coords[j+1][1]],[coords[j][2],coords[j+1][2]],'-', color = colour, lw = branch.getRadius())
    ...
    ax.set_axis_off()
    if path:
        plt.savefig(path + '/vis.png', bbox_inches='tight', dpi = 1000)
        plt.close()
    else:
        plt.show()

def plotSegments(col):
    fig = plt.figure()
    ax = mplot3d.Axes3D(fig)
    liver, mesh = getLiverSTL()
    ax.add_collection3d(liver)
    ...
    for seg in col.segList:
        segColor = segmentColor(seg)
        prox = seg.getProximal()
        dist = seg.getDistal()
        ax.plot3D([prox[0],dist[0]],[prox[1],dist[1]],[prox[2],dist[2]], color = segColor, lw = seg.getRadius())
...
def plotSegmentFilling(col, time, conc, path = None):
    fig = plt.figure()
    ax = mplot3d.Axes3D(fig)
    liver, mesh = getLiverSTL()
    ax.add_collection3d(liver)
    ...
    bound_mesh(ax, mesh)
    ax.view_init(elev = -180., azim = -90.)
    ax.text(25,0,175,'Concentration: %.3f' %(conc))
    ax.text(25,0,150,"Time: %.2f s" %(time))
    ax.set_axis_off()
...
def segmentColor(segment):
    conc = segment.getConcentration()
    R = 1 * (1-conc)
    G = 0
    B = 1 * conc
